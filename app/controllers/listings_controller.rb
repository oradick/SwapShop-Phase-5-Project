class ListingsController < ApplicationController
    skip_before_action :authorized, only: [:index, :create, :destroy, :update, :show, :my_listings, :listings_i_claimed, :listings_home]
    ...
    def listings_home
        listings = Listing.all.where("creator_id = recipient_id").or(Listing.all.where(offer: false))
    ...
    def listings_i_claimed
        my_claims = Listing.all.where.not(creator_id: session[:user_id]).and(Listing.all.where(recipient_id: session[:user_id]))
